#!/bin/bash
# run_cam1_imu.sh  ——  单独标定 cam1 与 IMU 的外参 (跳过相机内参标定)
# 用法 (容器内):  bash run_cam1_imu.sh /data/<folder>
#
# 与 run_cam0_imu.sh 唯一区别: 联合标定的相机由 cam0 换成 cam1。
#   从现成四目 camchain 里取 cam1, 去掉 T_cn_cnm1 (cam1 相对 cam0 的外参,
#   单相机-IMU 标定不需要), 让 cam1 作为独立链头喂给 cam-imu 联合标定。
#
# 与 run_one.sh 的区别:
#   run_one.sh:  步骤1 IMU内参 -> 步骤2 转bag -> 步骤3 四目相机标定 -> 步骤4 camN-IMU
#   本脚本:      步骤1 IMU内参 -> 步骤2 转bag ->  (跳过步骤3)   -> 步骤4 cam1-IMU
#   即不再自己跑 kalibr_calibrate_cameras, 而是直接接收一份现成的四目 camchain.yaml,
#   从中取 cam1 喂给 cam-imu 联合标定。相机内外参由外部给定。
#
# 重要: /data 是 goosefs 对象存储 FUSE, 只支持"整文件写入(cp)", 不支持 append/重写。
#       因此所有中间计算都在容器本地 /work 下完成, 最后把结果 cp 回 <folder>/result_cam1_imu/ (数据目录, 已挂载)。
#
# 期望输入:
...
# 产出 (cp 到 <folder>/result_cam1_imu/, 即宿主机 <DATA_ROOT>/<folder>/result_cam1_imu/):
#   imu.yaml
#   kalibr_input-camchain-cam1.yaml       (从四目 camchain 提取的 cam1)
#   kalibr_input-camchain-imucam.yaml     (cam1-imu 外参, 最终结果)
#   *_imu_param.yaml, *.log, *.pdf        (过程文件)
set -o pipefail

DATA_FOLDER="${1:?用法: run_cam1_imu.sh /data/<folder>}"
...
RESULT_DST="$DATA_FOLDER/result_cam1_imu"   # 最终结果落地 (数据目录下, 已挂载 -> 宿主机直接可见)

WORK="/work/${NAME}_cam1_imu"               # 容器本地工作区
...
if [ -f "$RESULT_DST/.done" ]; then
    log "已存在 result_cam1_imu/.done, 跳过 (删除可重跑)"
    exit 0
fi
...
############################################
# 步骤 3: (跳过) 从现成四目 camchain 提取 cam1
############################################
# 不跑 kalibr_calibrate_cameras。直接从外部给定的四目 camchain 里取 cam1,
# 去掉 T_cn_cnm1 (cam1 相对 cam0 的外参, 单相机-imu 标定不需要) 并清空 cam_overlaps。
# 注意: Kalibr 要求 camchain 的相机键必须从 cam0 起连续编号 (它按 cam0,cam1... 递增读),
#       所以这里把键名重命名成 cam0 (内容仍是 cam1 的内参; rostopic 仍是 /cam1/image_raw,
#       这才是决定实际用哪一路图像的关键)。
CAMCHAIN_IMUCAM_SRC="$OUTDIR/kalibr_input-camchain-cam1.yaml"
python3 -c "
import yaml, sys
d = yaml.safe_load(open('$CAMCHAIN'))
if 'cam1' not in d:
    sys.stderr.write('camchain 里没有 cam1 键, 现有: %s\n' % list(d.keys())); sys.exit(1)
c1 = d['cam1']
c1.pop('T_cn_cnm1', None)   # 去掉 cam1 相对 cam0 的外参, 让 cam1 成为独立链头
c1['cam_overlaps'] = []
# 键名必须是 cam0 (Kalibr 从 cam0 起读), rostopic 保持 /cam1/image_raw 指向真正的 cam1 图像
yaml.safe_dump({'cam0': c1}, open('$CAMCHAIN_IMUCAM_SRC','w'), default_flow_style=False, sort_keys=False)
" || { err "从 camchain 提取 cam1 失败"; exit 40; }
log "步骤3(跳过相机标定): 已从现成 camchain 提取 cam1 -> camchain-cam1.yaml"

############################################
# 步骤 4: Camera-IMU 联合标定 (仅 cam1)
############################################
IMUCAM="$OUTDIR/kalibr_input-camchain-imucam.yaml"
log "步骤4: kalibr_calibrate_imu_camera (cam1-imu)"
( cd "$OUTDIR" && \
